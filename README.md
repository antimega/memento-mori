...
- **tags** (`tags.html`) — a tag navigator for the Flickr archive: every tag as a clickable chip (most-used first, with a filter box), showing the selected tag's photos and videos. Tag selections are linkable (`tags.html#tag=venice`).
- **albums** (`albums.html`) — the same for Flickr albums: every album as a chip (most recent first, filterable), showing the selected album's photos and videos; the viewer links each photo's albums here. Selections are linkable (`albums.html#album=<id>`).
